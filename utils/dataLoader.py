...
class PeanutDataset(Dataset):
    def __init__(self, root, transform, classify):
        super(PeanutDataset, self).__init__()
        self.root = root
...
def load_data(batch_size, crop_size, classify):
    stats = ((0.1491, 0.1267, 0.9329), (0.2985, 0.2504, 0.1456))
    ...
    train_iter = torch.utils.data.DataLoader(PeanutDataset('./data/dataset/train', train_transform, classify), batch_size, shuffle=True, drop_last=True)
    test_iter = torch.utils.data.DataLoader(PeanutDataset('./data/dataset/validation', test_transform, classify), batch_size, shuffle=False, drop_last=True)
...
if __name__ == '__main__':
    ...
    train_iter, test_iter = load_data(128, 64, classify)
